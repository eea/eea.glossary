...
from eea.glossary.testing import IS_PLONE_5
...
class UpgradeTestCaseBase(unittest.TestCase):
    ...
    def setUp(self, from_version, to_version):
        ...
        self.profile_id = u'eea.glossary:default'
...
class Upgrade1to2TestCase(UpgradeTestCaseBase):
    ...
    def test_update_resource_conditions(self):
        ...
        JS_IDS = (
            '++resource++eea.glossary/tooltip.js',
            '++resource++eea.glossary/jquery.glossarize.js',
            '++resource++eea.glossary/main.js',
        )
        css_tool = api.portal.get_tool('portal_css')
        CSS_IDS = ('++resource++eea.glossary/tooltip.css',)

        # simulate state on previous version
        for id in JS_IDS:
            js = js_tool.getResource(id)
            js.setExpression(
                "python:portal.restrictedTraverse('@@glossary_state')()")
        for id in CSS_IDS:
